...
4. Do not mine `tasks.md` for behaviour.
5. If the spec and code leave a design boundary unclear, inspect the matching
   `plan.md` section narrowly for technical context only.
6. Walk the selected baseline's implementation and validation artefacts to
   build a behaviour inventory keyed by capability or owning component.
7. Read ratified architecture, governance, and equivalent decision records
   where present.
...
5. Normalise ubiquitous language across modules; if a term is used with two
   meanings, pick the one matching the code and rename the other.
6. Strip every reference to: phases, sprints, branches, work packages,
   feature numbers, "MVP", task IDs, "Phase 1/2", or implementation order.
7. Cite each requirement with at least one source spec section **and** one
...
- Appendix A maps every product ID back to its per-feature origin IDs.
