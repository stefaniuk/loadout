...
Create (or update) domain analysis documents at `docs/architecture/domain-*.md`.
...
For each domain, create `docs/architecture/domain-[XXX]-[name].md`, where:
...
In `docs/architecture/README.md`, add (or update) a **Domain Context Map** section with a single diagram showing all domains and the integration patterns between them.
...
Update `docs/architecture/README.md` with a **Domain Analysis** section linking to every domain document (in `[XXX]` order), plus the context map.
